...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE        
        embeddings = tf.reshape(tf.nn.embedding_lookup(tf.convert_to_tensor(self.pretrained_embeddings,tf.float32),self.input_placeholder),(-1,self.config.n_features*self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        ## create params
        W = tf.get_variable(name='W', shape=(self.config.n_features*self.config.embed_size,self.config.hidden_size), dtype=tf.float32, initializer=xavier_weight_init())
        b1 = tf.get_variable(name='b1', shape=(self.config.hidden_size,), dtype=tf.float32)
        
        ## add one layer
        W_e = tf.get_variable(name='W_e', shape=(self.config.hidden_size, self.config.hidden_size), dtype=tf.float32, initializer=xavier_weight_init())
        b_e = tf.get_variable(name='b_e', shape=(self.config.hidden_size,), dtype=tf.float32)
        
        
        U = tf.get_variable(name='U', shape=(self.config.hidden_size,self.config.n_classes), dtype=tf.float32, initializer=xavier_weight_init())
        b2 = tf.get_variable(name='b2', shape=(self.config.n_classes,), dtype=tf.float32)
        
        ## add l2 regularization
        self.l2_loss = tf.nn.l2_loss(W)+tf.nn.l2_loss(W_e)+tf.nn.l2_loss(U)
        h = tf.nn.relu(tf.matmul(x,W) + b1)
        ## add one layer
        h_e = tf.nn.relu(tf.matmul(h,W_e) + b_e)
        h_drop = tf.nn.dropout(h_e,keep_prob=self.dropout_placeholder)
