...
def minibatch_parse(sentences, model, batch_size):
    ...
    ### YOUR CODE HERE
    ## create pp object for each sentences
    partial_parses = [PartialParse(s) for s in sentences]
    unfinished_parses = partial_parses[:]
    ## init dependencies to be empty
    #dependencies = []
    while len(unfinished_parses) is not 0:
        ## use model to predict the first batch_size transitions        
        transitions = model.predict(unfinished_parses[:batch_size])
        
        ## run one parse step on each transitions
        for i in range(len(transitions)):
            unfinished_parses[i].parse_step(transitions[i])
        ## remove fulled parsed items
        remove_items = []
        for p in unfinished_parses[:batch_size]:
            if len(p.stack) == 1 and len(p.buffer) == 0:
                remove_items.append(p)
        for p in remove_items:
